...
export function PartnerIaErpPage() {
  const searchParams = useSearchParams();
  const initialAssistant = searchParams.get("assistant") ?? "admin";
  ...
  useEffect(() => {
    void (async () => {
      const res = await fetch("/api/partner/erp/ia", { credentials: "include", cache: "no-store" });
  ...
  const send = useCallback(async () => {
    ...
    setReply(null);
    try {
      const res = await fetch("/api/partner/erp/ia/chat", {
        method: "POST",
        credentials: "include",
  ...
  return (
    <div className="space-y-4">
      <PartnerErpModulePanel config={PARTNER_ERP_MODULE_CONFIG.ia} />
      <section className="mx-4 mb-6 rounded-2xl border border-zinc-200/80 bg-white p-5 dark:border-white/10 dark:bg-zinc-900/60 sm:mx-6">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="font-semibold">Assistentes IA</h2>
        </div>
        {configured === false ? (
          ...
          <>
            <div className="mb-3 flex flex-wrap gap-2">
              {ALL_PARTNER_AI_ASSISTANTS.map((a) => (
                <Button
                  key={a.id}
                  size="sm"
