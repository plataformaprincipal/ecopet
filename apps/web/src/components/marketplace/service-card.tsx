    ...
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-ecopet-gray/10 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg dark:bg-white/5">
      <Link href={`/marketplace/servico/${service.id}`} className="relative block aspect-video overflow-hidden bg-gray-100">
        ...
        <div className="absolute left-2 top-2 flex flex-wrap gap-1">
          <Badge className="bg-ecopet-dark text-white">Serviço</Badge>
          {service.emergency && <Badge className="bg-red-500 text-white">Emergência</Badge>}
          {service.aiTag && (
      ...
      <div className="flex flex-1 flex-col p-3 lg:p-4">
        ...
        <div className="mt-2 flex items-baseline gap-2">
          <span className="text-xs text-ecopet-gray">A partir de</span>
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-ecopet-gray">
          ...
          {service.homeService && (
            <span className="flex items-center gap-0.5"><Home className="h-3 w-3" /> Domicílio</span>
          )}
        ...
        {!compact && (
          <div className="mt-auto flex gap-1.5 pt-3">
            ...
            <Link href={`/marketplace/personalizados?service=${service.id}`}>
              <Button size="sm" variant="outline" className="px-2.5" aria-label="Orçamento">
            ...
            <Button
              ...
              onClick={() => toggleCompare("service", service.id)}
              aria-label="Comparar"
            >
