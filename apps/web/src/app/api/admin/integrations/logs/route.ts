...
import { getAdminIntegrationLogsModule } from "@/lib/platform/integration-automation-service";

export async function GET(request: Request) {
  const { error } = await requireAdmin({ path: "/api/admin/integrations/logs" });
  if (error) return error;
  try {
    const filters = parseGestorFilters(new URL(request.url).searchParams);
    return apiSuccess(await getAdminIntegrationLogsModule(filters));
  } catch (e) {
